...
/* *** BEGIN OBFUSCATION MAGIC - NSCOIN **********
    ...
    if (fMasterNode) {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat NSCOIN and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 NSCOIN per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100NSCOIN+1 ( bit on if present )
    // bit 1 - 10NSCOIN+1
    // bit 2 - 1NSCOIN+1
    // bit 3 - .1NSCOIN+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("nscoin-obfuscation");
