...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake) {
...

        {
            ...
            pblock->nBits = GetNextWorkRequired(pindexPrev, pblock->nTime, pblock);
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("NSCOINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("NSCOINMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("NSCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nscoin-miner");
    ...
    //control the amount of times the client will check for mintable coins
        ...
        LogPrintf("Running NSCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
